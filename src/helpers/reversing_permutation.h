...
struct reversing_permutation {
    ...
    static constexpr size_type factor = playground::power(Width, Level - 1);
    ...
    static constexpr size_type permute_idx(size_type idx)
    {
        ...
        constexpr size_type shift = playground::power(Width, Level - 1);
...
private:
    ...
    struct template_tile_permuter {
        ...
        {
            assert(I < playground::power(Width, Level));
            assert(permute_idx(I) < playground::power(Width, Level));
    ...
    {
        constexpr size_type num = playground::power(Width, Level);
...
public:
    ...
    {
        impl<playground::power(Width, Level)>(tile);
    }

    template <typename T, size_type N>
    void operator()(std::array<T, N>& tile) const noexcept
    {
        static_assert(N == playground::power(Width, Level), "N must correspond to Width and Level");
        impl(tile);
    }
    ...
    {
        impl<playground::power(Width, Level)>(src, dst, signer);
    }
...
public:
    ...
    void operator()(T* __restrict tile) const noexcept
    {
        impl<playground::power(Width, 2)>(tile);
    }
